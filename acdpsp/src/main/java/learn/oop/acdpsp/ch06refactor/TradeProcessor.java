...
public class TradeProcessor {
    ...
    public void processTrades(InputStream stream) throws IOException, SQLException {
        ...
        for (String line : lines) {
            ...
            if (fields.length != 3) {
                System.out.printf("WARN: %s malformed. Only %d field(s) found.%n", lineCount, fields.length);
                continue;
            }
            if (fields[0].length() != 6) {
                System.out.printf("WARN: Trade currencies on line %d malformed: '%s'%n", lineCount, fields[0]);
                continue;
            }
            Optional<Integer> tradeAmountOptional;
            if ((tradeAmountOptional = tryParseInteger(fields[1])).isEmpty()) {
                System.out.printf("WARN: Trade amount on line %d not a valid integer: '%s'%n", lineCount, fields[1]);
            }
            Optional<BigDecimal> tradePriceOptional;
            if ((tradePriceOptional = tryParseDecimal(fields[2])).isEmpty()) {
                System.out.printf("WARN: Trade price on line %d not a valid decimal: '%s'%n", lineCount, fields[2]);
            }
        ...
        System.out.printf("INFO: %d trades processed%n", trades.size());
    }
